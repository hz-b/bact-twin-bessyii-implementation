...
from bact_twin_bessyii_impl.bl.translator_service import TranslatorService
from ..bl.liaison_manager import LiaisonManager

from ..config.data.querries import get_magnets
from ..config.data.constants import ring_parameters, cavity_names
...
def element_method(element_name, yp: FamilyTree):
    d = dict(quadrupoles="K", sextupoles="H",
             horizontal_steerers="x_kick", vertical_steerers="y_kick")
    for family_name, prop in d.items():
        names = yp.get(family_name)
...
def extract_host_element_name(element_name: str, yp: FamilyTree) -> str:
