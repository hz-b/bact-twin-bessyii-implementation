...
from .bessyii_nomen_clature import name_matches_steerer_name, name_matches_horizontal_steerer_name, \
...
class LiaisonManager(LiaisonManagerBase):
    ...
    def master_clock_to_cavities(self, id_: DevicePropertyID) -> Sequence[LatticeElementPropertyID]:
        assert self._check_if_master_clock_name(id_.device_name)
        if id_.property != "reference_frequency":
            raise AssertionError(f"{self.__class__.__name__}: for master clock I am handling only frequency now")

        cavity_names = ["CAVH4T8R", "CAVH3T8R", "CAVH2T8R", "CAVH1T8R"]
        return [
            LatticeElementPropertyID(element_name=name, property="frequency") for name in cavity_names
        ]
